...
fn build(sysroot: Option<&str>) -> io::Result<()> {
    ...
    if env::var("DEBUG").is_ok() {
        ...
        // openconvert: upstream typo `-03` (invalid) -> `-O3`. gcc/clang reject
        // `-03`, which makes every FFmpeg configure compile probe fail and abort
        // with "pthreads requested but not found". See [patch.crates-io] in the
